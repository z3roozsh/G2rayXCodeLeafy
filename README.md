...
- `G2RAY_XHTTP_EXTRA_JSON` **(Optional)** — Supply the entire XHTTP `extra` object verbatim for the exported links (must be valid JSON), e.g. `{"xmux":{"hKeepAlivePeriod":15,"hMaxRequestTimes":600},"scMaxEachPostBytes":1000000}`. Overrides the default keepalive-only extra. Use it to experiment with additional xmux/transport fields. Regenerate and re-import after changing it.
- `G2RAY_XHTTP_KEEPALIVE` / `G2RAY_XHTTP_KEEPALIVE_SEC` **(Optional)** — Whether exported links carry an XHTTP `extra` (`xmux.hKeepAlivePeriod`) keepalive, and its period in seconds (default on, `30`). This makes the client send HTTP/2 PING frames so GitHub's edge does not idle-close the tunnel session — the root cause of "a page/app stops loading until I reopen it". xmux also consolidates streams, so fewer idle sessions can go stale. Set `G2RAY_XHTTP_KEEPALIVE=0` if a specific client cannot import a link that contains the `extra` parameter, then regenerate. After changing either value, regenerate and re-import.
